...
window.onload = function () {
  ...
  searchBtn.addEventListener("click", function () {
    ...
    fetch(
      `https://api.spoonacular.com/food/ingredients/search?query=${searchText}&addChildren=false&apiKey=${apiKey}`,
      { method: "get" }
    )
      ...
      .then(function (data) {
        ...
        return fetch(
          `https://api.spoonacular.com/food/ingredients/${ingredientId}/information?amount=1&unit=serving&apiKey=${apiKey}`,
          { method: "get" }
        );
      })
      .then(function (response) {
  ...
  refineSearchBtn.addEventListener("click", function () {
    ...
    if (
      ...
      alert(
        "make sure unit parameters are set and a record exists in the table before refining search"
      );
      return;
    }
  ...
  altSearchBtn.addEventListener("click", function () {
    ...
    fetch(
      `https://api.spoonacular.com/recipes/findByNutrients?maxCalories=${firstSearchResult.calories}&maxCarbs=${firstSearchResult.carbs}&apiKey=${apiKey}`
    )
      .then(function (response) {
        return response.json();
      })
